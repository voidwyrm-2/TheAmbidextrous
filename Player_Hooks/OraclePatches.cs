...
namespace TheAmbidextrous.Player_Hooks
{
    internal class OraclePatches
    {
        /*
        ...
        private static void PebblesConversation_AddEvents(On.SSOracleBehavior.PebblesConversation.orig_AddEvents orig, SSOracleBehavior.PebblesConversation self)
        {
            ...
            if (self.id == AmbidexterityEnums.SSOracle.Pebbles_Ambidextrous_FirstMeet)
            {
            ...
            else if (self.id == AmbidexterityEnums.Pebbles_Ambidextrous_AfterMet)
            {
            ...
            else if (self.id == AmbidexterityEnums.Pebbles_Ambidextrous_ExplainPackage)
            {
            ...
            else if (self.id == AmbidexterityEnums.Pebbles_Ambidextrous_ExplainPackageFirstMeet)
            {
                self.events.Add(new Conversation.TextEvent(self, 0, self.Translate(".  .  ."), 0));
                self.events.Add(new Conversation.TextEvent(self, 0, self.Translate("A little animal, on the floor of my chamber, is this reaching you?"), extralingerfactor * 80));
                self.events.Add(new Conversation.TextEvent(self, 0, self.Translate("It is obvious that you are not a product of natural evolution. I'm wondering who your creator are."), extralingerfactor * 80));
        ...
        private static void SSOracleBehavior_SeePlayer(On.SSOracleBehavior.orig_SeePlayer orig, SSOracleBehavior self)
        {
            ...
            {
                if (self.action != AmbidexterityEnums.SSOracle.MeetAmbidextrous)
                {
                    ...
                    self.NewAction(AmbidexterityEnums.SSOracle.MeetAmbidextrous);
                }
            }
            else
            {
        ...
        {
            if (nextAction == AmbidexterityEnums.SSOracle.MeetAmbidextrous)
            {
                if (self.currSubBehavior.ID == AmbidexterityEnums.SSOracle.Meet_DroneMaster) return;
                ...
                {
                    if (self.allSubBehaviors[i].ID == AmbidexterityEnums.Meet_DroneMaster)
                    {
                ...
                if (subBehavior == null) subBehavior = new SSOracleMeetAmbidextrous(self);
        ...
        public class SSOracleMeetAmbidextrous : SSOracleBehavior.ConversationBehavior
        {
            ...
            public SSOracleMeetAmbidextrous(SSOracleBehavior owner) : base(owner, AmbidexterityEnums.Meet_DroneMaster, AmbidexterityEnums.Pebbles_Ambidextrous_FirstMeet)
            {
            }
                ...
                if (!initMessage && !dialogBox.ShowingAMessage && dialogBox.messages.Count == 0)
                {
                    ...
                    if (oracle.room.game.GetStorySession.saveState.miscWorldSaveData.SSaiConversationsHad == 0)
                    {
                        if (scannedSaveUnit.KingScanned && !pebbleConvSaveUnit.explainPackage)
                        {
                            ...
                            owner.InitateConversation(AmbidexterityEnums.Pebbles_Ambidextrous_ExplainPackageFirstMeet, this);
                        }
                        else
                            owner.InitateConversation(AmbidexterityEnums.Pebbles_Ambidextrous_FirstMeet, this);
                    }
                    else
                    {
                        if (scannedSaveUnit.KingScanned && !pebbleConvSaveUnit.explainPackage)
                        {
                            pebbleConvSaveUnit.explainPackage = true;
                            owner.InitateConversation(AmbidexterityEnums.Pebbles_Ambidextrous_ExplainPackage, this);
                        }
                        else
                            owner.InitateConversation(AmbidexterityEnums.Pebbles_Ambidextrous_AfterMet, this);
                    }
